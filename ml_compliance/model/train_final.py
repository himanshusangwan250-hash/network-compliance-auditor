#!/usr/bin/env python3
"""
Final SetFit training script.
Trains from scratch and evaluates on test set.
"""
...
def run_cross_vendor_evaluation(model):
    """Run cross-vendor experiments (simplified - use main model)."""
    print("\n" + "=" * 80)
    ...
    for train_file in sorted(cv_dir.glob("train_*.csv")):
        ...
        # Use main trained model for prediction
        predictions = model.predict(test_texts)
        if hasattr(predictions, 'tolist'):
            predictions = predictions.tolist()
