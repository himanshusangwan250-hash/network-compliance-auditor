#!/usr/bin/env python3
"""
Model V3 Training Script.
Uses Dataset V3 (ml_compliance/data_v3/) and outputs to ml_compliance/model/setfit_v3/.
Keeps V1 and V2 models untouched.
"""
...
# ============================================================================
# Configuration — V3 paths (separate from V1 and V2)
# ============================================================================

MODEL_NAME = "all-MiniLM-L6-v2"
OUTPUT_DIR = Path(__file__).parent / "setfit_v3"          # V3 model output
DATA_DIR = Path(__file__).parent.parent / "data_v3"        # V3 dataset
RANDOM_SEED = 42
CONFIDENCE_THRESHOLD = 0.6

# Training hyperparameters (optimized for ~2000 examples)
EMBEDDING_EPOCHS = 2           # More epochs for larger dataset
CLASSIFIER_EPOCHS = 5          # Classification head training epochs
MAX_STEPS = 500                # Max training steps for embeddings
BATCH_SIZE = 8                 # Batch size
LEARNING_RATE = 2e-5           # Learning rate for body
HEAD_LEARNING_RATE = 1e-2      # Learning rate for head
MAX_LENGTH = 256               # Full context length


def setup_environment():
    """Print environment info and verify GPU."""
    print("=" * 80)
    print("MODEL V3 TRAINING ENVIRONMENT")
    print("=" * 80)
...
def train_model():
    """Train the SetFit model on V3 data with CUDA."""
    print("\n" + "=" * 80)
...
def evaluate_model(model, test_df):
    ...
    print("\n" + "=" * 80)
    print("MODEL V3 TEST EVALUATION")
    print("=" * 80)
...
def run_validation_evaluation(model, val_df):
    ...
    print("\n" + "=" * 80)
    print("MODEL V3 VALIDATION EVALUATION")
    print("=" * 80)
...
def save_results(model, test_results, val_results, training_time):
    ...
    # Save
    results_file = OUTPUT_DIR / "training_results_v3.json"
...
def inference_test(model, test_df):
    ...
    print("\n" + "=" * 80)
    print("INFERENCE COMPATIBILITY TEST (Model V3)")
    print("=" * 80)
    ...
    for idx in sample_indices[:min(5, len(test_df))]:
        ...
        status = '[PASS]' if prediction == row['label'] else '[FAIL]'
        print(f"  {status} Example {idx}: pred={prediction}, actual={row['label']}, "
...
def main():
    ...
    print("=" * 80)
    print("MODEL V3 TRAINING - Dataset V3 -> SetFit V3")
    print("=" * 80)
    ...
    print(f"Model saved to: {OUTPUT_DIR}")
    print(f"Results saved to: {OUTPUT_DIR / 'training_results_v3.json'}")
